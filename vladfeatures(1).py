...
vlad_feature_train, y_train = get_data_and_labels(train_set, codebook)
...
vlad_feature_test, y_test = get_data_and_labels(test_set, codebook)

# vlad_feature_test.resize(gabor_feature_test)
# B = (vlad_feature_test, gabor_feature_test)
# x_test = np.vstack(B)
x_test = vlad_feature_test
...
confusion_matrix = np.zeros((len(class_list), len(class_list)), dtype=np.uint32)
